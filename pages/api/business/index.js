...
import { uploadLogo } from '../../../helpers/functions';
...
handler.put(async (req, res) => {
    ...
    if (logo) {
      dest = uploadLogo(logo, session ? session.user.name : "guest");
    }

    const sql = //         
          `UPDATE business
          SET name='${name}', address='${address}', phone='${phone}',
            email='${email}'${dest ? `, logo='${dest}'` : JSON.parse(req.body.logo) ? "" : ', logo=null'}
          WHERE user='${session ? session.user.name : "guest"}'`;
...
//         //     uploadLogo(logo, session ? session.user.name : "guest");
